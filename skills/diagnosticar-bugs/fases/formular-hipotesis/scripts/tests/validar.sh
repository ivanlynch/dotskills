...
{
  registro H01 "$HIP_VALIDA"
  registro H02 "$HIP_VALIDA"
  registro H03 "$HIP_VALIDA"
...
nuevo_state "$state" \
  "$(registro H06 "$HIP_VALIDA")" \
  "$(registro H07 "$HIP_VALIDA")" \
  "$(registro H08 "$HIP_VALIDA")" \
  "JUSTIFICACION_MENOS_DE_3: no aplica"
salida=$(bash "$SCRIPT" "$state" 2>/dev/null) && rc=0 || rc=$?
if [ "$rc" -ne 0 ] || [ "$salida" != "READY" ]; then
