...
- Ejecutar `<skill-dir>/scripts/workflow_state.sh` para registrar todos los cambios de estado.
...
```bash
<skill-dir>/scripts/workflow_state.sh init <TICKET_ID>
<skill-dir>/scripts/workflow_state.sh show <TICKET_ID> --compact
```
...
6. Si la consulta es exitosa, registrar el contexto antes de terminar la fase:
   ...
   ```bash
   <skill-dir>/scripts/workflow_state.sh context <TICKET_ID> --id "<ticket_id>" --title "<title>" --description "<description>" --source jira
   <skill-dir>/scripts/workflow_state.sh done <TICKET_ID> ticket --jira-read --evidence "<TICKET_ID>: contexto cargado por consultar-ticket"
   ```
...
1. Verificar con `<skill-dir>/scripts/workflow_state.sh show <TICKET_ID> --compact` que `ticket_context=COMPLETE`.
2. Marcar `analizar-alcance` como `IN_PROGRESS`.
3. Leer por completo y seguir `<skill-dir>/references/analizar-alcance.md`.
...
7. Persistir el árbol completo con el controlador de estado (texto plano, sin JSON — ver `<skill-dir>/references/analizar-alcance.md`). Cada nodo debe tener `title`, `parent`, `status` y `verdict`; las hojas `APTO_PARA_IMPLEMENTAR` además necesitan `acceptance_criteria`.
...
11. Después de cada actualización, ejecutar:
    ...
    ```bash
    <skill-dir>/scripts/workflow_state.sh validate <TICKET_ID>
    ```

12. Registrar cada nodo con los comandos granulares de `workflow_state.sh` (`scope-set-objective`, `scope-add-item`, `scope-set-item`, `scope-set-criteria`) — no existe un comando único que reemplace todo el árbol de una vez. Para cerrar el análisis, ejecutar `scope-confirm <TICKET_ID>` recién cuando todos los nodos estén `DONE`.
...
2. Leer por completo y seguir `<skill-dir>/references/plan.md` con la guía local encontrada; lee el alcance confirmado directamente del registro de la fase `analizar-alcance`, no hace falta pasárselo. No reconstruir el alcance ni editar manualmente el estado del plan.
3. Dividir el trabajo en tareas ejecutables, con resultado observable, verificación concreta y orden por dependencia.
4. Redactar y persistir directamente el título y la descripción de cada etapa mediante `plan_state.sh`; no invocar `/crear-ticket` ni esperar confirmaciones por etapa.
5. No implementar nada. Marcar la fase como `DONE` solamente cuando la referencia devuelva `PLAN_COMPLETE`, haya creado el archivo ejecutable del plan y todas las tareas estén persistidas como `READY`.
...
```bash
<skill-dir>/scripts/workflow_state.sh done <TICKET_ID> aprobacion --user-approved --evidence "Aprobación explícita del usuario: <respuesta>"
```
...
2. Leer por completo y seguir `<skill-dir>/references/implementar.md` con el archivo ejecutable creado por la fase `plan`, siempre con commit por tarea.
3. Esa referencia ejecuta una tarea por vez y marca cada tarea como `DONE` en el archivo únicamente después de recibir evidencia `DONE`.
...
5. Marcar la fase como `DONE` solo cuando la referencia devuelva `PLAN_COMPLETE`; si devuelve `BLOCKED`, conservar la fase bloqueada con la evidencia.
